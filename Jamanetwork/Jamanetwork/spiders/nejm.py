...
class NejmSpider(scrapy.Spider):
    ...
    def parse_info(self, response):
        ...
        item['pub_date'] = response.xpath('//meta[@name="evt-dt"]/@content').get()
        item['abstract'] = strip_tag(response.xpath('//section[@class="o-article-body__section"]//text()').extract()) or\
                           strip_tag(response.xpath('//div[@class="m-letter"]//p[@class="f-body"]//text()')) or ''
        item['doi'] = response.xpath('//meta[@name="evt-doiPage"]/@content').get()
        item['authors'] = response.xpath('//header//ul[contains(@class,"m-article-header__authors")]//li//text()').extract()
